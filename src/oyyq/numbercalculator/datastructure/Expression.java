...
public class Expression {
	...
	public String toString(boolean fullParenthesis) {
		...
		if (operator != null) {
			...
			// calculates first
			// (a + b) * c
			if (!operand1.simple && (fullParenthesis || (operand1.operator.getPriority() < operator.getPriority()))) {
				sb.append(OPEN_PARENTHESIS).append(operand1.toString(fullParenthesis)).append(CLOSE_PARENTHESIS);
			} else {
			...
			if (!operand2.simple
					&& (fullParenthesis || ((operand2.operator.getPriority() < operator.getPriority() || (!operator
							.isExchangable() && operator.getPriority() == operand2.operator.getPriority()))))) {
				sb.append(OPEN_PARENTHESIS).append(operand2.toString(fullParenthesis)).append(CLOSE_PARENTHESIS);
			} else {
