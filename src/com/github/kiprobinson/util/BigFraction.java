...
public final class BigFraction extends Number implements Comparable<Number> {
	...
	public BigFraction add(Number n) {
		...
		if (isInt(n)) {
			...
			return new BigFraction(numerator.add(denominator.multiply(toBigInteger(n))), denominator, Reduced.YES);
		} else {
	...
	public BigInteger round(RoundingMode roundingMode) {
		...
		final Set<RoundingMode> ROUND_HALF_MODES = EnumSet.of(RoundingMode.HALF_UP, RoundingMode.HALF_DOWN,
	...
	public BigFraction subtract(Number n) {
		...
		if (isInt(n)) {
			// n1/d1 - n2 = (n1 - d1*n2)/d1
			return new BigFraction(numerator.subtract(denominator.multiply(toBigInteger(n))), denominator, Reduced.YES);
		} else {
			BigFraction f = valueOf(n);
