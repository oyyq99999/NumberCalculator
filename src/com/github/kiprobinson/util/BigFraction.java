...
/**
 * Arbitrary-precision fraction, utilizing BigIntegers for numerator and denominator. Fraction is
 * always kept in lowest terms. Fraction is immutable, and guaranteed not to have a null numerator
 * or denominator. Denominator will always be positive (so sign is carried by numerator, and a
 * zero-denominator is impossible).
 * 
...
public final class BigFraction extends Number implements Comparable<Number> {
    ...
    // because Number is Serializable
    private static final long       serialVersionUID = 7077748865051777066L;
    private final BigInteger        denominator;
    private final BigInteger        numerator;

    /**
     * Constructs a BigFraction from given number. If the number is not one of the primitive types,
     * BigInteger, BigDecimal, or BigFraction, then Number.doubleValue() will be used for
     * construction. Warning: when using floating point numbers, round-off error can result in
     * answers that are unexpected. For example, System.out.println(BigFraction.valueOf(1.1)) will
     * print: 2476979795053773/2251799813685248 This is because 1.1 cannot be expressed exactly in
     * binary form. The computed fraction is exactly equal to the internal representation of the
     * double-precision floating-point number. (Which, for 1.1, is: (-1)^0 * 2^0 * (1 +
     * 0x199999999999aL / 0x10000000000000L).) NOTE: In many cases,
     * BigFraction.valueOf(Double.toString(d)) may give a result closer to what the user expects.
     */
     ...
     * of how the values were passed in. Warning: when using floating point numbers, round-off error
     * can result in answers that are unexpected. For example,
     * System.out.println(BigFraction.valueOf(1.1)) will print: 2476979795053773/2251799813685248
     * This is because 1.1 cannot be expressed exactly in binary form. The computed fraction is
     * exactly equal to the internal representation of the double-precision floating-point number.
     * (Which, for 1.1, is: (-1)^0 * 2^0 * (1 + 0x199999999999aL / 0x10000000000000L).) NOTE: In
     * many cases, BigFraction.valueOf(Double.toString(d)) may give a result closer to what the user
     * expects.
     * 
    ...
    /**
     * Constructs a BigFraction from a String. Expected format is numerator/denominator, but
     * /denominator part is optional. Either numerator or denominator may be a floating- point
    ...
    /**
     * Returns true if the given type represents an integer (Long, Integer, Short, Byte, or
     * BigInteger). Used to determine if a Number is appropriate to be passed into toBigInteger()
     * method.
     */
    ...
    /**
     * Converts a Number to a BigInteger. Assumes that a check on the type of n has already been
     * performed.
     */
    ...
    private static BigFraction valueOfHelper(BigDecimal numerator, BigDecimal denominator) {
        ...
        if (numerator.scale() > denominator.scale())
            tmpDenominator = tmpDenominator.multiply(BigInteger.TEN.pow(numerator.scale()
                    - denominator.scale()));
        else if (numerator.scale() < denominator.scale())
            tmpNumerator = tmpNumerator.multiply(BigInteger.TEN.pow(denominator.scale()
                    - numerator.scale()));
    ...
    private static BigFraction valueOfHelper(double d) {
        ...
        //
        // SPECIAL CASE: Subnormals - if all exponent bits are 0 (in my code,
        // this
        ...
        BigInteger tmpNumerator = BigInteger.valueOf((isSubnormal ? 0L : 0x10000000000000L)
                + mantissa);
        BigInteger tmpDenominator = BigInteger.ONE;
        ...
        if (exponent > 52) {
            ...
            if (!isSubnormal) {
                // The gcd of (2^52 + mantissa) / 2^(52 - exponent) must be of
                // the form 2^y,
                // since the only prime factors of the denominator are 2. In
                // base-2, it is
                // easy to determine how many factors of 2 a number has--it is
                // the number of
                // trailing "0" bits at the end of the number. (This is the same
                // as the number
    ...
    /**
     * Constructs a BigFraction from two floating-point numbers. Warning: round-off error in IEEE
     * floating point numbers can result in answers that are unexpected. See BigFraction(double) for
     * more information. NOTE: In many cases, BigFraction(Double.toString(numerator) + "/" +
     ...
     * terms. No check is done to reduce numerator/denominator. A check is still done to maintain a
     * positive denominator.
     * 
     * @param isReduced
     *            Indicates whether or not the fraction is already known to be reduced to lowest
     *            terms.
     */
    ...
    public BigFraction add(Number n) {
        ...
        if (isInt(n)) {
            ...
            return new BigFraction(numerator.multiply(f.denominator).add(
                    denominator.multiply(f.numerator)), denominator.multiply(f.denominator),
                    Reduced.NO);
        }
    }

    /**
     * Returns a byte representation of this fraction. This value is obtained by integer division of
     * numerator by denominator. If the value is greater than Byte.MAX_VALUE, Byte.MAX_VALUE will be
     * returned. Similarly, if the value is below Byte.MIN_VALUE, Byte.MIN_VALUE will be returned.
     */
    @Override
     ...
     * to, or greater than f, respectively.
     */
    ...
    /**
     * Returns a negative, zero, or positive number, indicating if this object is less than, equal
     * to, or greater than n, respectively.
     */
    @Override
    ...
    /**
     * Returns the complement of this fraction, which is equal to 1 - this. Useful for
     ...
     * *only* return true if the other object is a BigFraction. For looser comparison to other
     * Number objects, use the equalsNumber(Number) method.
     */
    @Override
    ...
    /**
     * Returns the value of this fraction. If this value is beyond the range of a float,
     * Float.INFINITY or Float.NEGATIVE_INFINITY will be returned.
     */
    @Override
    ...
    /**
     * Returns an int representation of this fraction. This value is obtained by integer division of
     * numerator by denominator. If the value is greater than Integer.MAX_VALUE, Integer.MAX_VALUE
     * will be returned. Similarly, if the value is below Integer.MIN_VALUE, Integer.MIN_VALUE will
     * be returned.
     */
    @Override
    public int intValue() {
    ...
    // --------------------------------------------------------------------------
    /**
     * Returns a long representation of this fraction. This value is obtained by integer division of
     * numerator by denominator. If the value is greater than Long.MAX_VALUE, Long.MAX_VALUE will be
     * returned. Similarly, if the value is below Long.MIN_VALUE, Long.MIN_VALUE will be returned.
     */
    @Override
    ...
    /**
     * Returns this^exponent. Note: 0^0 will return 1/1. This is consistent with Math.pow(),
    ...
    /**
     * Returns this rounded to the nearest whole number, using RoundingMode.HALF_UP as the default
     * rounding mode.
     */
     ...
     *             if RoundingMode.UNNECESSARY is used but this fraction does not exactly represent
     *             an integer.
     */
    public BigInteger round(RoundingMode roundingMode) {
        ...
        final Set<RoundingMode> ROUND_HALF_MODES = EnumSet.of(RoundingMode.HALF_UP,
                RoundingMode.HALF_DOWN, RoundingMode.HALF_EVEN);
    ...
    /**
     * Returns a short representation of this fraction. This value is obtained by integer division
     * of numerator by denominator. If the value is greater than Short.MAX_VALUE, Short.MAX_VALUE
     * will be returned. Similarly, if the value is below Short.MIN_VALUE, Short.MIN_VALUE will be
     * returned.
     */
    @Override
     ...
     * the same number of significant figures as both numerator and denominator, or the equivalent
    ...
    public BigDecimal toBigDecimal() {
        ...
        if (BigInteger.ONE.equals(tmpDen)) {
            ...
            if (twos < fives)
                unscaled = unscaled.shiftLeft(fives - twos); // x * 2^n === x <<
                                                             // n
            else if (fives < twos)
    ...
    public BigDecimal toBigDecimal(int precision) {
        return new BigDecimal(numerator).divide(new BigDecimal(denominator), new MathContext(
                precision, RoundingMode.HALF_EVEN));
    }

    /**
     * Returns string representation of this object as a mixed fraction. For example, 4/3 would be
     * "1 1/3". For negative fractions, the sign is carried only by the whole number and assumed to
     * be distributed across the whole value. For example, -4/3 would be "-1 1/3". For fractions
     * that are equal to whole numbers, only the whole number will be displayed. For fractions which
     * have absolute value less than 1, this will be equivalent to toString().
     */
